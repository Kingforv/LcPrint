...
public class LabelBarcodeActivity extends BaseActivity implements RadioGroup.OnCheckedChangeListener, View.OnClickListener, PrinterBinderListener {
    ...
    public void printBarcode(String barContent) {
        try {
            ...
            if ("1".equals(mPageType)) {
                printUtil.setFeedPaperSpace(distance);//标签模式下生效
                printUtil.setUnwindPaperLen (return_distance);
            }
            printUtil.addBarcode(PrintConfig.Align.ALIGN_CENTER, 100, barContent, mBarcode1DType, PrintConfig.HRIPosition.POSITION_BELOW);
    ...
    public void printQR(String barContent) {
        try {
            ...
            printUtil.addQRCode(PrintConfig.Align.ALIGN_CENTER, 384, barContent);
            if ("2".equals(mPageType)) {
                printUtil.addLineFeed(10);
            }
            printUtil.start();
